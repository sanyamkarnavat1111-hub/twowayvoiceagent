...
import { Mic, Database, Search, MessageSquare, Settings2, Sparkles, ShieldCheck } from "lucide-react";
import { BotSettings, BotState } from "../types.js";

interface VoiceBotHeaderProps {
  activeTab: "voice" | "transcript" | "kb" | "vector-search";
  setActiveTab: (tab: "voice" | "transcript" | "kb" | "vector-search") => void;
  botState: BotState;
      ...
      case "thinking":
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold rounded-full bg-indigo-500/10 text-indigo-600 border border-indigo-500/20 animate-pulse">
            ...
            {botState === "retrieving" ? "Vector RAG Searching..." : "Reasoning..."}
          </span>
        );
      case "speaking":
        return (
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-full bg-slate-100 text-slate-700 border border-slate-200">
            <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
            VoiceBot Ready ({totalDocs} Docs Indexed)
          </span>
        );
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="hidden sm:inline-block px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center gap-1">
                  <ShieldCheck className="w-3 h-3 text-emerald-600" /> Pure TypeScript SPA
                </span>
              </div>
              <p className="text-xs text-slate-500">Zero-Exhaustion Client Vector Database & Two-Way Voice</p>
            </div>
          </div>
